...
table_query_base_filters = {
  'ATOM':  u"AssessmentDate ge @lower and AssessmentDate lt @upper and IsActive eq 1 and Program ne 'TEST' and Status eq 'Complete'",
  
  # 'ATOM_Matching':
  #  u"AssessmentDate ge @lower and AssessmentDate lt @upper and IsActive eq 1 and Program ne 'TEST' and Status eq 'Complete'",
...
def get_results(table:str, start_date, end_date, fields:list[str], filters:dict|None={}) -> list[dict]:
    
    stq = SampleTablesQuery(table)    
    
    all_filters = table_query_base_filters.get(table, "")
    # if not tconfig:
